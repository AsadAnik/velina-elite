...
export default function CategoryPage({ params }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    switch (sort) {
      ...
      case "rating": result.sort((a, b) => b.rating - a.rating || b.reviewCount - a.reviewCount); break;
    }
    return result;
    ...
    return (
      <>
        ...
        <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4 bg-background">
          <p className="text-2xl font-black text-zinc-800 dark:text-zinc-200">Category not found</p>
          <Link href="/" className="text-velina-rose font-bold underline">Back to Home</Link>
        </div>
        <CartDrawer />
      </>
    );
  ...
  return (
    <>
      ...
      <div className="bg-gradient-to-r from-zinc-800 to-zinc-700 dark:from-zinc-950 dark:to-zinc-900 py-8 px-4 text-center">
        <h1 className="text-2xl md:text-3xl font-black text-white tracking-wide">{category.name}</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 flex gap-6 bg-background min-h-screen">
        <CategorySidebar currentCategory={category} totalCount={allProducts.length} />
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mb-4 flex-wrap">
            <span className="flex items-center gap-1.5 bg-velina-peach/40 dark:bg-velina-rose/10 text-velina-rose text-xs font-bold px-3 py-1.5 rounded-full border border-velina-rose/20">
              ...
              <Link href="/" className="hover:opacity-70"><X size={12} /></Link>
            </span>
          ...
          {filtered.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
              ...
              <button onClick={() => setSearch("")} className="text-velina-rose text-sm font-bold underline">Clear search</button>
            </div>
          )}
        </div>
      </div>
